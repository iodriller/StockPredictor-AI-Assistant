...
TRADER_USE: dict[str, str] = {
    "action": "This is entry readiness after safety checks. A bullish bias can still correctly say wait for market open or skip a poor entry.",
    ...
    "vwap": "Use true session VWAP as the intraday 'fair price'. On the daily chart, the rolling volume-weighted average is only a broader reference.",
...
GLOSSARY: list[tuple[str, str]] = [
    ...
    ("VWAP", "Volume-Weighted Average Price — a session-anchored intraday fair-value line. A rolling daily-bar volume-weighted average is a broader proxy, not true session VWAP."),
    ...
    ("Backtest evaluation", "A historical moment where the strategy asks whether a fresh setup passes its signal and risk gates."),
    ("Warm-up bars", "Historical bars gathered before the first backtest decision so indicators and models have enough context."),
    ("No-trade rate", "The share of historical evaluations that correctly stayed out instead of opening a position."),
    ...
    ("Trading lens", "A named evidence blend for the setup you are researching: balanced, price-action, catalyst-led, trend/swing, news stress-test, or custom."),
    ("Entry readiness", "Whether a fresh entry is ready to review, waiting for the market to open, watching for confirmation, or should be skipped."),
...
WORKSPACE_GUIDES: dict[str, dict] = {
    ...
    "backtest": {
        ...
        "what": "A simulation of the repeatable price, technical, and risk path on past daily bars. It excludes historical news because the free providers do not supply point-in-time archives. This is a logic check, NOT proof of future profit.",
        "look_at": ["number of evaluations and executed trades", "Account return with max drawdown", "No-trade rate and dominant skip reasons", "individual trade outcomes"],
        "steps": [
            "Start with one symbol and Standard review.",
            "Check sample quality first. Fewer than 10 executed trades is too thin for performance conclusions.",
            "Read account return together with drawdown and skipped-setup reasons.",
            "Use Deeper sample when the standard run produces too little evidence.",
            "Treat results skeptically — past ≠ future.",
        ],
    },
    "journal": {
